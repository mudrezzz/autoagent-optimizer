...
def _build_demo_bindings() -> RendererBindings:
    ...
    prompt_templates = {
        ...
        "style_rewrite_direct_v1": (
            "Перепиши пост в живом человеческом стиле для LinkedIn/Telegram.\n"
            "Сохрани факты, цифры и последовательность мысли.\n"
            "Убери канцелярит и шаблонные AI-фразы.\n\n"
            "Исходный пост:\n{draft_post}"
        ),
        "style_rewrite_with_patterns_v1": (
            "Ты редактор соцсетей. Перепиши текст так, чтобы он звучал естественно и лично.\n"
            "Сохрани фабулу, факты и числа.\n"
            "Снижай повтор шаблонов. Если есть `cleaned_text`, используй его как базу.\n\n"
            "Черновик:\n{cleaned_text}\n\n"
            "Оригинал (fallback):\n{draft_post}\n\n"
            "Найденные AI-паттерны:\n{ai_pattern_hits}"
        ),
        "style_finalize_v1": (
            "Финально отредактируй пост: убери сухость и штампы, но не меняй факты и цифры.\n\n"
            "Текущая версия:\n{text}"
        ),
    }
