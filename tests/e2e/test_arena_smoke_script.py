"""E2E-тест smoke-скрипта турнира Architecture Arena."""
...
@pytest.mark.e2e
def test_arena_smoke_script_passes() -> None:
    """Проверяет успешный e2e запуск smoke-сценария Arena турнира."""

    shell_bin = _resolve_powershell()
    if shell_bin is None:
        pytest.skip("PowerShell не найден в окружении тестов.")

    script_path = _project_root() / "scripts" / "smoke_run_arena.ps1"
    proc = subprocess.run(
