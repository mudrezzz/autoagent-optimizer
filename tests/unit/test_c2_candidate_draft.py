...
def test_candidate_draft_builder_returns_deterministic_shape() -> None:
    ...
    assert payload["project_id"] == "prj_demo_1"
...
def test_candidate_draft_builder_rejects_invalid_input() -> None:
    """Проверяет валидацию пустого brief и некорректного max_candidates."""

    with pytest.raises(ValueError):
        build_candidate_draft_from_brief(project_id="prj_demo_1", brief="   ", max_candidates=3)
    with pytest.raises(ValueError):
        build_candidate_draft_from_brief(project_id="prj_demo_1", brief="ok", max_candidates=0)
    with pytest.raises(ValueError):
        build_candidate_draft_from_brief(project_id="prj_demo_1", brief="ok", max_candidates=6)
